...
class ScrapeChannelDataView(View):
    ...
    def scrape_channel_data(self, request, handle):
        try:
            ...
            url = f"https://t.me/{handle}"  # https://t.me/joinchat/g565_dZMo4E4M2I0
            ...
            return render(
                request,
                self.template_name,
                {"data": data, "counter": counter, "media_form": media_form},)
...
class StoreChannelData:
    ...
    def save_data(request, data, counter):
        ...
        try:
            ...
            error_message = "Media already exists in the database."
            return render(
                self.request, "err.html", {"error_message": error_message})  # ! improve error message
        except Media.DoesNotExist:
        ...
        return redirect(
            reverse("success"))  # Redirect to a success page after saving the data
