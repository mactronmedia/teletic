...
class ChannelScraper:
    def scrape_channel_data(self, handle):
        ...
        file_path, thumbnail_file_path = self.download_avatar(handle, channel_avatar)  # Create an instance and call download_avatar
    ...
    def scrape_created_date(self, handle):
        url = f"{TELEGRAM_URL}/s/{handle}/1"
        headers = {"User-Agent": user_agent.random}
        page = session.get(url, headers=headers)
        soup = BeautifulSoup(page.content, "lxml")
    ...
    def download_avatar(self, handle, channel_avatar):
...
class SaveChannelData:
    def save_data(self, request, handle, data, created, counter):
