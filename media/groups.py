...
class ScrapeGroupDataView(View):
    ...
    def post(self, request, handle):
        ...
        return self.group_exists(request, handle)

    def group_exists(self, request, handle):
        try:
            Media.objects.get(handle=handle)
            error_message = "Media already exists in the database."
            return render(request, self.template_name, {"error_message": error_message})
        except Media.DoesNotExist:
            group_data = self.scrape_group_data(handle)
            if "error_message" in group_data:  # Check if an error message was returned
                return render(request, self.template_name, {"error_message": group_data["error_message"]})

            request.session["data"] = group_data
    ...
    def scrape_group_data(self, handle):
        ...
        file_path, thumbnail_file_path = self.download_photo(handle, group_photo)
    ...
    def download_photo(self, handle, group_photo):
        file_path = settings.LOGO_PHOTO
        thumbnail_file_path = settings.THUMBNAIL_PHOTO

        if ".jpg" in group_photo:
            current_month = datetime.now().month
            directory = f"{current_month:02d}"
            folder_path = os.path.join("storage/groups", directory)
